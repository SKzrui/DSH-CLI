...
class SplitScreen {
  ...
  /** 1-based row of the fixed thinking indicator (just above the input box). */
  indicatorRow() {
  ...
  writeLine(line) {
    ...
    // While busy, flow one row higher so the indicator row stays free.
    for (let k = 0; k < n + (busy ? 1 : 0); k++) process.stdout.write("\n");
    activeEditor?.drawInput();
...
class LineEditor {
  ...
  drawInput() {
    ...
    const cols = Math.max(1, split.cols);
    // Each logical line occupies its own row(s): long lines wrap INSIDE the
    // box (the terminal wraps them), so advance by the wrapped row count and
    // never clear a row the wrap continuation is sitting on.
    let row = 0;
    for (let i = 0; i < parts.length && row < n; i++) {
      process.stdout.write(`\x1b[${top + row};1H\x1b[2K`);
...
function makeRenderer() {
  ...
  const renderTable = () => {
    ...
    const termW = Math.max(40, (process.stdout.columns || 80) - 2);
